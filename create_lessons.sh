...
for filepath in `ls -1 | awk -F '.' '{print $1}' | fgrep -v index | uniq`; do
  ...
  echo "  <a href='"$path".pptx'><img src='http://archive.ev3lessons.com/web/EV3lessons-EV3Trainer-v1/advanced/Calibrate/powerpoint.svg'  width='30' height='30'> $path - Microsoft Powerpoint</a><br>" >> $OUTPUT
  echo "  <a href='"$path".pdf'><img src='http://cwidaho.cc/sites/all/themes/omega_cwi/images/icons/adobe-pdf-icon.png'  width='30' height='30'> $path - Microsoft Powerpoint</a><br>" >> $OUTPUT
#  echo "  <UL>" >> $OUTPUT
#  for i in `find "$filepath" -maxdepth 1 -mindepth 1 -type f| sort`; do
#    file=`basename "$i"`
#    echo "    <LI><a href=\"./$path/$file\">$file</a></LI>" >> $OUTPUT
